...
# 数据准备
IM_WIDTH, IM_HEIGHT = 299, 299 #InceptionV3指定的图片尺寸299*299
...
#添加最后的层输入base_model和分类数量输出新的keras的model
    x = base_model.output 
    x = GlobalAveragePooling2D()(x) 
    x = Dense(FC_SIZE, activation='relu')(x) #new FC layer, random init 
    #x = Dense(512, activation='relu')(x) #new FC layer, 512
    #x = Dense(256, activation='relu')(x) #new FC layer, 256
...
def setup_to_transfer_learn(model, base_model): 
    ...
    model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy']) # 定义网络框架 
...
# 模式一训练 
history_tl = model.fit_generator( train_generator, nb_epoch=nb_epoch, samples_per_epoch=nb_train_samples, 
    validation_data=validation_generator,workers=2, nb_val_samples=nb_val_samples, class_weight='auto')


model.save('./weights/Inception.h5')
